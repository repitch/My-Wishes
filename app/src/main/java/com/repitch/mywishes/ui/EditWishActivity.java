...
public class EditWishActivity extends AppCompatActivity {
    ...
    public static Intent createIntent(Context context, int wishId) {
        return new Intent(context, EditWishActivity.class)
            ...
            editImageUrlDialog.setListener(new EnterTextDialog.OnEnterTextListener() {
                ...
                public boolean onTextEntered(String text) {
                    ...
                    if (!TextUtils.isEmpty(wish.getImageUrl())) {
                        Glide.with(EditWishActivity.this)
    ...
    private void saveWish() {
        Single.fromCallable(() -> {
            ...
            String link = wishLink.getText().toString();
            Uri linkUri = Uri.parse(link);
            if (linkUri.getScheme() == null) {
                link = "http://" + link;
            }
            wish.setLink(link);
            return wishRepository.updateWish(wish);
        })
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(wish -> {
                    if (createWish) {
                        Toast.makeText(EditWishActivity.this, "Желание создано!", Toast.LENGTH_SHORT).show();
                        startActivity(WishDetailActivity.createIntent(this, wish.getId()));
                        finish();
                    } else {
                        Toast.makeText(EditWishActivity.this, "Желание сохранено!", Toast.LENGTH_SHORT).show();
                        setResult(RESULT_OK);
                        finish();
                    }
                }, Timber::e);
